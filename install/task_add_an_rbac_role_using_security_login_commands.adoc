...
NOTE: Para consistência, a documentação do SnapCenter refere-se às funções como usando o Privileges. A GUI do OnCommand System Manager usa o termo "atributo" em vez de "privilégio". Ao configurar funções do ONTAP RBAC, ambos esses termos significam a mesma coisa.
